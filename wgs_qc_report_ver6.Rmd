...
wgs_samplesheet$sample_id <- paste(wgs_samplesheet$sample_name, batch_name_clean, sep = "_")
...
wgs_result_df$sample_id <- sub("^(([^_]+_){2}).*", "\\1", wgs_result_df$sample_id)
...
summary_table <- function(data) {
  ...
  #remove batch name in sample_id
  data$sample_id <- sub("^(([^_]+_){2}).*", "\\1", data$sample_id)
  # Remove trailing underscore
...
reco_df <- wgs_df %>%
  ...
  mutate(
    ...
    sample_id = sub("^(([^_]+_){2}).*", "\\1", sample_id),
...
#remove batch name in sample_id
mlst_df_clean$sample_id <- sub("^(([^_]+_){2}).*", "\\1", mlst_df_clean$sample_id)
# Remove trailing underscore
...
#remove batch name in sample_id
df_amrfinderplus1$sample_id <- sub("^(([^_]+_){2}).*", "\\1", df_amrfinderplus1$sample_id)
# Remove trailing underscore
...
for(i in amr_gene_df_list) {
  ...
  #remove batch name in sample_id
  wgs_species_1$sample_id <- sub("^(([^_]+_){2}).*", "\\1", wgs_species_1$sample_id)
  # Remove trailing underscore
