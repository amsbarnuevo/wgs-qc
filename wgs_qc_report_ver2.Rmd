...
reco_df <- bind_rows(
  ...
  mutate(sample_id = sub("(.*)_[^_]*$", "\\1", sample_id)) %>%   # 👈 normalize IDs
  group_by(sample_id) %>%
  summarise(
...
qc_pass <- nrow(bactscout_df_result[bactscout_df_result$a_final_status == "PASSED", ])
qc_failed <- ifelse(length(qc_not_identified) == 0, nrow(bactscout_df_result[bactscout_df_result$a_final_status == "FAILED", ]), 
                    nrow(bactscout_df_result[bactscout_df_result$a_final_status == "FAILED", ]) + qc_not_identified)
...
qc_concordance <- nrow(wgs_df[wgs_df$concordant == TRUE, ])
...
# Convert class
assembly_qc_checked$total_contig <- as.numeric(unlist(assembly_qc_checked$total_contig))

# Create a new variable for color mapping
assembly_qc_checked$status <- ifelse(assembly_qc_checked$total_contig < 500, "Passed", "Above threshold")

# Plot
ggplot(data = assembly_qc_checked) +
...
# Convert class
assembly_qc_checked$n50_contig_length <- as.numeric(unlist(assembly_qc_checked$n50_contig_length))

ggplot(data = assembly_qc_checked) +
  geom_point(mapping = aes(x = n50_contig_length, y = org_code_clean, color = ifelse(n50_contig_length_qc == "PASSED" , "#8ad879", "#f3533a"))) +
  ylab("Organisms") +
  xlab("N50 Value") +
...
ggplot(data = assembly_qc_checked) +
  geom_point(mapping = aes(x = total_contig, y = org_code_clean, color = ifelse(total_contig_qc == "PASSED" , "#8ad879", "#f3533a"))) +
  ylab("Organisms") +
...
assembly_qc_checked <- assembly_qc_checked %>%
  ...
  mutate(
    condition_checks = list(c(
      ...
      "total_coding_sequences" = total_coding_sequences_qc == "PASSED",
      "genome_size" = genome_size_qc == "PASSED",
      "gc_content" = gc_content_qc == "PASSED",
      "n50" = n50_contig_length_qc == "PASSED",
      "no_of_contigs" = total_contig_qc == "PASSED"
    )),
...
assembly_qc_value <- assembly_qc_checked %>%
  select(iso_num, sample_id, wgs_id, completeness, contamination, total_coding_sequences, 
        genome_size, gc_content, n50_contig_length, total_contig) %>%
  mutate(sample_id = sub("(.*)_[^_]*$", "\\1", sample_id))
...
colnames(assembly_qc_value) <- c("Isolate No.","Sample ID", "WGS ID", "Completeness", "Contamination", "Total Coding Sequences",
                                 "Genome Size", "GC Content", "N50", "Total Contig")
    ...
    # Split and convert to numeric safely
      ...
      for (col in indices) {
        ...
        if (display_col <= ncol(assembly_qc_value)) {
          ...
          # Apply LaTeX formatting for PDF
          assembly_qc_value[i, display_col] <- paste0("\\colorbox{myred}{\\textcolor{white}{\\textbf{",as.character(original_value),"}}}")
...
colnames(assembly_reco_df) <- c('Sample ID','Reason - Failed Metrics','Remarks')
