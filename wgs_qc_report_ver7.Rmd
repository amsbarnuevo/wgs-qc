...
wgs_df$Group <- ifelse((wgs_df$ORG == 'sal' & wgs_df$wgs_id == 'Salmonella enterica subsp. enterica serovar Typhi'),"Salmonella enterica, typhoidal", ifelse((wgs_df$ORG == 'sal' & wgs_df$wgs_id != 'Salmonella enterica subsp. enterica serovar Typhi'),"Salmonella enterica, nontyphoidal", wgs_df$wgs_id))


#identify salmonella typhi and salmonella nontyphi
wgs_df$ORG <- ifelse((wgs_df$ORG == 'sal' & wgs_df$Group == 'Salmonella enterica, typhoidal'), 'sat', wgs_df$ORG)
...
retain_column <- c('iso_num','sample_id','wgs_id','description','Group')
...
colnames(qc_listing_df) <- c('Sample ID','ARSRL','Isolate No.','WGS','Description','Group')

qc_listing_df1 = subset(qc_listing_df, select = -c(Group) )

qc_listing_df1 <- qc_listing_df1[, c(3,1,5,2,4)] 
...
qc_summary_df <- qc_listing_df %>% group_by(Group) %>% summarise(number = sum(!is.na(WGS)))
...
wgs_id_count <- table(wgs_df$Group)
...
if (nrow(wgs_lowreads) != 0 ){
  sample_id <- c("24ARS_NKI0054", "22ARS_VSM0456")
  action <- c("Repeat testing", "Repeat testing")
  reason <- c("Low read count", "QC failure (high contigs)")
...
wgs_species <- subset(wgs_df, , select = c("sample_id", "wgs_id", "Group"))
mlst_df_clean <- merge(mlst_df_clean,wgs_species,by = "sample_id")

#reorder column
mlst_df_clean <- mlst_df_clean %>% 
  relocate(wgs_id, .after = species) %>%
  relocate(Group, .after = wgs_id)

#identify salmonella enterica group
mlst_df_clean$species <- ifelse(mlst_df_clean$Group == 'Salmonella enterica, nontyphoidal', 'salmonella_enterica_nontyphoidal', ifelse(mlst_df_clean$Group == 'Salmonella enterica, typhoidal','salmonella_enterica_typhoidal', mlst_df_clean$species))
...
for(i in mlst_df_list) {
  df <- get(i)
  df <- subset(df, select = -c(species, Group) )
...
for(i in mlst_df_list) {
    ...
    mlst_count_df <- df %>% group_by(Group,MLST) %>% summarise(count = sum(!is.na(MLST)))
    ...
    mlst_count_summary  <- subset(mlst_count_df , select = c(Group, mlst_count))
...
for(i in amr_gene_df_list) {
  ...
  names(df)[names(df) == 'Group'] <- 'species'
  ...
  #subset dataframe to retain needed columns only
  df <- subset(df, select = -c(species, wgs_id))
