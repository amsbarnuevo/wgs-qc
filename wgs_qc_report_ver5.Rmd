...
#remove ".fna.gz"
mlst_df_clean$sample_id <- gsub(".fna.gz", "", mlst_df_clean$sample_id, fixed=TRUE)
...
names(df_amrfinderplus) <- tolower(names(df_amrfinderplus))


#Change Name to sample_id
names(df_amrfinderplus)[names(df_amrfinderplus) == 'name'] <- 'sample_id'
names(df_amrfinderplus)[names(df_amrfinderplus) == 'element symbol'] <- 'element_symbol'

#change "-" to "_" in sample_id column
df_amrfinderplus$sample_id <- gsub("-", "_", df_amrfinderplus$sample_id, fixed=TRUE)

#merge wgs_id from wgs_df to mlst_df to set as species value
df_amrfinderplus <- merge(df_amrfinderplus,wgs_species,by = "sample_id")

#remove "quasi" to wgs_id column
df_amrfinderplus$wgs_id <- gsub("quasi", "", df_amrfinderplus$wgs_id, fixed=TRUE)

amr_genes_species <- unique(df_amrfinderplus$wgs_id)
...
for(i in amr_genes_species) {
  df <- subset(df_amrfinderplus, df_amrfinderplus$wgs_id == i)
  sample_id <- c(df$sample_id)
  
  #create new dataframe to duplicate df_amrfinderplus
  amr_genes_df <- df_amrfinderplus
...
#list existing sample_id in the mlst table
no_amr <- wgs_df$wgs_id[!(wgs_df$wgs_id %in% df_amrfinderplus$wgs_id)]
...
if (length(no_amr) !=0){
  sample_id <- wgs_df$sample_id[!(wgs_df$sample_id %in% df_amrfinderplus$sample_id)]
  wgs_id <- no_amr
