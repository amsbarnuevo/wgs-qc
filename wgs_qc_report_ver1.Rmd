...
#wgs_samplesheet$sample_id <- gsub('_', '\\_', wgs_samplesheet$sample_id, fixed=TRUE)
#wgs_samplesheet$sample_name <- gsub('_', '\\_', wgs_samplesheet$sample_name, fixed=TRUE)

#wgs_samplesheet$sample_id <- str_replace_all(wgs_samplesheet$sample_id, fixed("_"), "\\_")
#wgs_samplesheet$sample_name <- str_replace_all(wgs_samplesheet$sample_name, fixed("_"), "\\_")
...
#wgs_samplesheet_select <- subset(wgs_samplesheet, select = c('sample_id','description'))
...
qc_table <- qc_listing_df
#qc_table[['Sample ID']] <- str_replace_all(qc_table[['Sample ID']], fixed("_"), "\\_")
...
if (nrow(wgs_lowreads != 0)){
  ...
  colnames(lowreads_table) <- c('Sample ID','Description','Index reads','Remarks') 
...
wgs_summary_table <- wgs_summary_df
#wgs_summary_table$sample_id <- str_replace_all(wgs_summary_table$sample_id, fixed("_"), "\\_")

colnames(wgs_summary_table) <- c('Isolate No.','Sample ID','Contamination','Contigs', 'GC Percent','N50','Total Length')
...
#warning_summary_df$cols_warning <- str_replace_all(warning_summary_df$cols_warning, fixed("_"), "\\_")
#qc_listing_df <- qc_listing_df[, c(3,1,5,2,4)] 

if (nrow(warning_summary_df !=0)){
  warning_summary_table <- warning_summary_df
  #warning_summary_table$sample_id <- str_replace_all(warning_summary_table$sample_id, fixed("_"), "\\_")
  colnames(warning_summary_table) <- c('Isolate No.','Sample ID','Value with warning/s') 
...
qc_failed_count <- nrow(QC_failed)
...
qc_failed <- nrow(wgs_df[wgs_df$result == 'FAILURE', ])
...
-   $\color{red}`r qc_pass_pc`$ % (n=`r qc_pass`) of the isolates passed the QC, while $\color{red}`r qc_warning_pc`$ % (n=`r qc_warning`) were tagged with warning and $\color{red}`r qc_failed_pc`$ % (n=`r qc_failed`) were tagged with failure.
...
if ((nrow(wgs_lowreads) != 0 ) | (qc_failed_count != 0)){
  sample_id <- c('24ARS_CRH0096')
  action <- c('Check isolate label')
  reason <- c('Non-concordant result')
