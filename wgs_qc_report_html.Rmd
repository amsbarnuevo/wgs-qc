...
```{r sunburst-graph, echo = FALSE, warning = FALSE, message = FALSE, fig.height=7, fig.width=7}
...
***Figure 1. *** **Distribution and quality control results of sequenced isolates.**
$\\$ $\\$

***Table 1.*** **Concordance analysis of bacterial identification of sequenced isolates.**
...
if (nrow(wgs_lowreads) != 0) {
  ...
  sketch <- htmltools::withTags(table(
    ...
    thead(
      tr(
        th(colspan = 3, style = "border-bottom: 1px solid #111;", "Sample excluded in the analysis")
      ),
      tr(
...
***Table 2.*** **Summary of raw reads quality control analysis.**
...
if(length(no_bactscout_result) != 0){
  ...
  sketch <- htmltools::withTags(table(
    ...
    thead(
      tr(
        th(colspan = ncol(df_no_bactscout), 
           style = "font-style: bold; font-size: 16px; color: black;", 
           "Summary of No Raw Reads Data")
        ),
      tr(
        lapply(colnames(df_no_bactscout), th, style = "text-align: center;")
        )
      )
  ))
  ...
  datatable(df_no_bactscout,
    ...
    options = list(
      ...
      columnDefs = list(
        list(targets = 0, className = 'dt-center'),  # Sample IDs left aligned
...
sketch <- htmltools::withTags(table(
  ...
  thead(
    tr(
      th(colspan = ncol(summary_raw_df), 
         style = "font-style: bold; font-size: 16px; color: black;", 
         "Summary of Raw Reads with FAILED QC results")
      ),
    tr(
      lapply(colnames(summary_raw_df), th, style = "text-align: center;")
      )
    )
))
  ...
  datatable(summary_raw_df,
            ...
            options = list(
              ...
              columnDefs = list(
                ...
                list(targets = 1, className = 'dt-center') # center aligned
              ),
              initComplete = JS(
                "function(settings, json) {",
                "$(this.api().table().header()).css({'background-color': '#D4D4D4', 'color': 'black'});",
...
***Table 3.*** **Summary of assembly reads quality control analysis.**
...
sketch <- htmltools::withTags(table(
  ...
  thead(
    tr(
      th(colspan = ncol(assembly_reco_df), 
         style = "font-style: bold; font-size: 16px; color: black;", 
         "Summary of Assembly Reads with FAILED QC results")
      ),
    tr(
      lapply(colnames(assembly_reco_df), th, style = "text-align: center;")
      )
    )
))
...
### **Final Recommendation based on QC results**
...
***Table 4.*** **Summary of Multilocus Sequence Typing (MLST) results of** ***`r paste(mlst_organism_list, collapse = ", ")`***
...
if (length(no_mlst) != 0) {
  ...
  sketch <- htmltools::withTags(table(
    ...
    thead(
      tr(
        th(colspan = ncol(df_no_mlst), 
           style = "font-style: bold; font-size: 16px; color: black;", 
           "Summary of Sample with No MLST Result")
        ),
      tr(
        lapply(colnames(df_no_mlst), th, style = "text-align: center;")
        )
      )
  ))
...
***Table 5.*** **Summary of AMR genes present among of** ***`r paste(amr_genes_list, collapse = ", ")`***
    ...
    thead(
      tr(
        th(colspan = ncol(df_subset), 
           style = "font-style: italic; font-size: 16px; border-bottom: 1px solid #111;", 
           species_val)
      ),
      tr(
