...
wgs_df$org_code[grepl("Streptococcus pseudopneumoniae",wgs_df$wgs_id)] = "spn"
...
wgs_summary_result1 <- wgs_summary_result1 %>%
  mutate(sample_id = paste0(sample_id, "_", batch_name_clean)) %>%
...
#file_name <- paste0("data_files/",batch_code,"_wgs_df_result.xlsx")
#write_xlsx(wgs_summary_result1, file_name)
...
wgs_qc$failed_conditions = wgs_df$failed_conditions[match(qc_listing_nc$sample_id, wgs_df$sample_id)]


if (nrow(wgs_no_result != 0)){
  no_result <- wgs_no_result$sample_id
  wgs_qc <- wgs_qc %>%
    mutate(
      concordant = ifelse(sample_id %in% no_result, TRUE, concordant),
      failed_conditions = ifelse(sample_id %in% no_result, "No WGS Result", failed_conditions)
    )
...
#remove batch name in sample_id
df_amrfinderplus1 <- df_amrfinderplus1 %>%
  mutate(
    sample_id = case_when(
      str_count(sample_id, "_") == 4 ~ sub("^([^_]+_[^_]+_[^_]+_[^_]+)_.*$", "\\1", sample_id),
      str_count(sample_id, "_") == 3 ~ sub("^([^_]+_[^_]+_[^_]+)_.*$", "\\1", sample_id),
