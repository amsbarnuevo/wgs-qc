...
wgs_df$org_code_clean[grepl("Serratia marcescens",wgs_df$wgs_id)] = "sma"
wgs_df$org_code_clean <- ifelse(wgs_df$wgs_id == "Escherichia/Shigella", "eco", wgs_df$org_code_clean)
...
wgs_samplesheet_subset <- subset(wgs_samplesheet, select = c('sample_id','description'))
...
arsrl_result_df$sample_id <- paste(arsrl_result_df$sample_id, batch_name_clean, sep = "_")


#merge  and samplesheet
arsrl_result_df <- merge(unique(arsrl_result_df), unique(wgs_samplesheet_subset), by = "sample_id", all.x= TRUE) 



#merge ARSRL result and WGS result
qc_listing_df <- merge(unique(wgs_result_df), unique(arsrl_result_df), by = "sample_id", all.x= TRUE) 

# Extract the desired order from df2
ordered_vector <- wgs_result_df$sample_id

# Reorder df1 based on the order_id column matching the ordered_vector
qc_listing_df <- qc_listing_df[match(ordered_vector, qc_listing_df$sample_id), ]
...
if (nrow(wgs_no_result != 0)){
  qc_listing_df$description <- ifelse(is.na(qc_listing_df$description),wgs_samplesheet_subset$description[match(paste0(wgs_no_result$sample_id, "_", batch_name_clean), wgs_samplesheet_subset$sample_id)], qc_listing_df$description)
...
qc_listing_nc <- qc_listing_df %>%
  rowwise() %>%
  mutate(concordant = ifelse(wgs_id != "Not Identified", grepl(Genus, wgs_id), TRUE))


# Result with no referred data
qc_listing_nc$concordant <- ifelse(qc_listing_nc$arsrl_org == "No Referred Data", TRUE, qc_listing_nc$concordant)
...
qc_listing_df$sample_id <- sub("(.*)_[^_]*$", "\\1", qc_listing_df$sample_id)

#qc_listing_df$sample_id <- gsub("_", "\\_", qc_listing_df$sample_id, fixed = TRUE)


# rename columns
colnames(qc_listing_df) <- c('Sample ID','WGS','ARSRL','Description','Isolate No.')


# arrange columns
qc_listing_df <- qc_listing_df[, c(5,1,4,3,2)] 
...
retain_column <- c('iso_num','sample_id','wgs_id','completeness','contamination','final_coverage','total_contig_length','gc_content','total_contig','n50_contig_length','combined_qual_mean', 'condition_checks')
...
summary_table <- function(data) {
  ...
  #remove batch name in sample_id
  data$sample_id <- sub("(.*)_[^_]*$", "\\1", data$sample_id)

  data$sample_id <- gsub("_", "\\_", data$sample_id, fixed = TRUE)
  data$combined_qual_mean <- format(round(unlist(data$combined_qual_mean), 1), nsmall = 1)
  ...
  display_df <<- data %>%
    ...
    rename(`Isolate No.` = iso_num,
           ...
           `Mean read Q-score` = combined_qual_mean) %>%
    select(-col_index)
...
wgs_summary_result1 <- merge(wgs_summary_result, wgs_summary_df, by = "sample_id")


wgs_summary_result1 <- wgs_summary_result1 %>%
  select(-iso_num, -col_index) %>%
  relocate(batch_code, .before = sample_id)


# append googlesheet "result" tab
sheet_append(ss = sheet_id, sheet = "result", data = wgs_summary_result1)
```
...
wgs_qc$failed_conditions = wgs_df$failed_conditions[match(wgs_qc$sample_id, wgs_df$sample_id)]
...
if (nrow(wgs_lowreads) != 0){
  ...
  # arrange columns
  wgs_lowreads_reco <- wgs_lowreads_reco[, c(2,1,3)] 
...
qc_not_identified <- qc_summary_df$number[qc_summary_df$WGS == "Not Identified"]


qc_tl <- ifelse(length(qc_not_identified) == 0, nrow(qc_listing_df), nrow(qc_listing_df) - qc_not_identified)
qc_distinct <- ifelse(length(qc_not_identified) == 0, nrow(qc_summary_df), nrow(qc_summary_df) - 1)

qc_pass <- nrow(wgs_df[wgs_df$result == TRUE, ])
qc_failed <- ifelse(length(qc_not_identified) == 0, nrow(wgs_df[wgs_df$result == FALSE, ]), nrow(wgs_df[wgs_df$result == FALSE, ]) + qc_not_identified)

qc_overall <- ifelse(length(qc_not_identified) == 0, qc_tl, qc_tl + qc_not_identified)

qc_pass_pc <- if (qc_pass != 0) (qc_pass/qc_overall) * 100 else 0
qc_pass_pc <- format(round(qc_pass_pc, 2), nsmall = 2)

qc_failed_pc <- if (qc_failed != 0) (qc_failed/qc_overall) * 100 else 0
qc_failed_pc <- format(round(qc_failed_pc, 2), nsmall = 2)

qc_concordance <- nrow(qc_listing_nc[qc_listing_nc$concordant == TRUE, ])
qc_concordance <- if (length(qc_not_identified) != 0) qc_concordance - qc_not_identified else qc_concordance
...
summary_id <- qc_summary_df
colnames(summary_id) <- c('WGS_ID','Frequency')

ggplot(data = summary_id, aes(x = " ", y = Frequency, fill = WGS_ID)) +
...
ggplot(data = wgs_df, aes(x = org_code_clean, fill = result_cat)) +
  geom_bar() +
...
# Plot
ggplot(data = wgs_df) +
  geom_point(mapping = aes(x = total_contig, y = org_code_clean, color = status)) +
  ylab("Organisms") +
...
ggplot(data = wgs_df) +
  geom_point(mapping = aes(x = n50_contig_length, y = org_code_clean, color = ifelse(n50_contig_length > 20000 , "#8ad879", "#f3533a"))) +
  ylab("Organisms") +
  xlab("N50 Value") +
...
ggplot(data = wgs_df) +
  geom_point(mapping = aes(x = total_contig_length, y = org_code_clean, color = ifelse(genome_size_result == "passed" , "#8ad879", "#f3533a"))) +
  ylab("Organisms") +
